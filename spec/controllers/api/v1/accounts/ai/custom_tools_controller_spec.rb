...
RSpec.describe 'Api::V1::Accounts::Ai::CustomToolsController', type: :request do
  ...
  def create_tool(payload, as_user: admin)
    post "/api/v1/accounts/#{account.id}/ai/assistants/#{assistant.id}/custom_tools",
         params: payload, headers: as_user.create_new_auth_token, as: :json
  end

  it 'creates a tool scoped to the agent' do
    expect do
      create_tool(ai_custom_tool: { title: 'Buscar', endpoint_url: 'https://loja.example.com/api',
  ...
  it 'never returns the auth credentials' do
    create_tool(ai_custom_tool: { title: 'Buscar', endpoint_url: 'https://loja.example.com/api',
                                  http_method: 'GET', auth_type: 'basic',
                                  auth_config: { username: 'u', password: 'segredo' }, param_schema: [] })
  ...
  it 'is administrator-only' do
    create_tool({ ai_custom_tool: { title: 'x', endpoint_url: 'https://loja.example.com/api' } }, as_user: agent)
