  ...
  describe '#execute' do
    ...
    context 'when matching responses exist' do
      before do
        ...
        allow(Captain::AssistantResponse).to receive(:search).with(anything, account_id: assistant.account_id).and_return([response])
      end
    ...
    context 'when no matching responses exist' do
      before do
        allow(Captain::AssistantResponse).to receive(:search).with(anything, account_id: assistant.account_id).and_return([])
      end
