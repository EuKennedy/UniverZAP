...
class Kanban::Automations::Actions::MoveToFunnel < Kanban::Automations::Actions::Base
  ...
  def perform!
    target_funnel = resolve_target_funnel!
    target_stage = resolve_target_stage!(target_funnel)
  ...
  def resolve_target_funnel!
    funnel = account.funnels.find_by(id: required_param!(:funnel_id).to_i)
    return funnel if funnel

    raise ExecutionError, "funnel_id=#{params[:funnel_id]} not on account"
  end

  def resolve_target_stage!(target_funnel)
    stage = target_funnel.funnel_stages.find_by(id: required_param!(:stage_id).to_i)
    return stage if stage

    raise ExecutionError, "stage_id=#{params[:stage_id]} not in funnel=#{target_funnel.id}"
  end
end
