module SortHandler
  ...
  PINNED_FIRST_PREFIX = 'pinned_at IS NULL, pinned_at DESC'

  class_methods do
    def sort_on_last_activity_at(sort_direction = :desc)
      order(generate_sql_query("#{SortHandler::PINNED_FIRST_PREFIX}, last_activity_at #{sort_direction.to_s.upcase}"))
    end

    def sort_on_created_at(sort_direction = :asc)
      order(generate_sql_query("#{SortHandler::PINNED_FIRST_PREFIX}, created_at #{sort_direction.to_s.upcase}"))
    end

    def sort_on_priority(sort_direction = :desc)
      order(generate_sql_query("#{SortHandler::PINNED_FIRST_PREFIX}, priority #{sort_direction.to_s.upcase} NULLS LAST, last_activity_at DESC"))
    end

    def sort_on_priority_created_at(sort_direction = :desc)
      order(generate_sql_query("#{SortHandler::PINNED_FIRST_PREFIX}, priority #{sort_direction.to_s.upcase} NULLS LAST, created_at ASC"))
    end

    def sort_on_waiting_since(sort_direction = :asc)
      order(generate_sql_query("#{SortHandler::PINNED_FIRST_PREFIX}, (waiting_since IS NULL), waiting_since #{sort_direction.to_s.upcase}, created_at ASC"))
    end
